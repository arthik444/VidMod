    ...
    return (
        <div className="flex flex-col h-full bg-card">
            <div className="p-4 border-b border-border bg-muted/20 flex items-center justify-between">
                <h3 className="font-semibold text-xs tracking-tight flex items-center gap-2">
                    <Scissors className="w-4 h-4 text-primary/50" />
                    Remediation Plan
                </h3>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-0 custom-scrollbar relative">
                ...
                <div className="mb-4 rounded border border-border bg-secondary/10 p-3">
                    <button
                        onClick={() => setShowCustomInput(!showCustomInput)}
                        className="w-full flex items-center gap-2 text-xs font-semibold text-muted-foreground hover:text-foreground transition-colors uppercase tracking-wider"
                    >
                        <Plus className="w-3.5 h-3.5" />
                        Add Translation / Edit
                        <ChevronDown className={`w-3.5 h-3.5 ml-auto transition-transform ${showCustomInput ? 'rotate-180' : ''}`} />
                    </button>
                    ...
                    {customObjects.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-border/50">
                            ...
                            <button
                                ...
                                className="w-full py-2 bg-secondary hover:bg-secondary/80 text-foreground border border-border rounded text-[10px] font-semibold uppercase tracking-widest transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                            >
                                {isProcessingBatch ? (
                                    <>
                ...
                {steps.length > 0 && jobId && (
                    <div className="mt-6 mb-4">
                        <button
                            ...
                            className="w-full py-2 bg-foreground text-background hover:bg-foreground/90 rounded font-semibold text-[10px] uppercase tracking-widest transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        >
                            {isProcessingBatch ? (
                                <>
                                ...
                                <>
                                    Process All Findings ({steps.length})
                                </>
                            )}
                        </button>
                        {batchProgress && (
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div className="relative z-10 w-full max-w-4xl max-h-[90vh] bg-card border border-border rounded-2xl shadow-2xl overflow-hidden flex flex-col">
                        ...
                        <div className="flex items-center justify-between p-4 border-b border-border bg-secondary/20">
                            <h2 className="font-semibold text-sm flex items-center gap-2 uppercase tracking-wider text-muted-foreground">
                                Batch Pipeline Editor
                            </h2>
                        ...
                        <div className="p-4 border-t border-border bg-secondary/10 flex items-center justify-between">
                            <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">
                                {batchConfigs.filter(c => c.selected).length} / {batchConfigs.length} Selected
                            </span>
                            <div className="flex gap-2">
                                <button
                                    onClick={() => setShowBatchReviewModal(false)}
                                    className="px-4 py-2 bg-transparent hover:bg-secondary text-foreground rounded font-semibold text-[10px] uppercase tracking-widest transition-all border border-border"
                                >
                                    Cancel
                                </button>
                                <button
                                    ...
                                    className="px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground rounded font-semibold text-[10px] uppercase tracking-widest transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-none"
                                >
                                    Ingest & Process ({batchConfigs.filter(c => c.selected).length})
                                </button>
                            </div>
                        </div>
                    </div>
